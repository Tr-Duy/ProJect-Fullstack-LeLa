...
# Fetch flashcards per level for questions
...
quiz_id_counter = max(max_quiz_id + 1, 35000)
question_id_counter = max(max_question_id + 1, 500000)
option_id_counter = max(max_option_id + 1, 1500000)
...
# Deactivate any duplicate level up quizzes created in previous scratch runs
sql_lines.append("\n-- Disable extra level up quizzes from past scratch runs")
sql_lines.append("UPDATE quizzes SET is_active = 0 WHERE quiz_category = 'LEVEL_UP' AND id > 10000;")
sql_lines.append("DELETE FROM quizzes WHERE quiz_code LIKE 'UPGRADE-L1-%';")

# 1. Update Existing Level 2 Upgrade Tests (IDs 171-180 -> level_id = 2, quiz_code = 'UPGRADE-U500-01'..'10', Title = 'Bài kiểm tra nâng cấp TOEIC 500 - 700 - #XX')
sql_lines.append("\n-- 1. Update Level 2 Upgrade Tests (level_id = 2, target 500-700)")
for i in range(1, 11):
    q_id = 170 + i
    q_code = f"UPGRADE-U500-{i:02d}"
    q_title = escape_sql(f"Bài kiểm tra nâng cấp TOEIC 500 - 700 - #{i:02d}")
    q_desc = escape_sql(f"Bài kiểm tra nâng cấp trình độ TOEIC 500 - 700 - Đề số #{i:02d}. Đạt từ 80% (24/30 câu) để nâng cấp trình độ ngay lập tức.")
    sql_lines.append(
        f"UPDATE quizzes SET level_id = 2, quiz_code = '{q_code}', title = '{q_title}', description = '{q_desc}', quiz_category = 'LEVEL_UP', pass_score = 80, time_limit_seconds = 1800, total_questions = 30, is_active = 1 WHERE id = {q_id};"
    )

# 2. Update Existing Level 3 Upgrade Tests (IDs 181-190 -> level_id = 3, quiz_code = 'UPGRADE-500-01'..'10', Title = 'Bài kiểm tra nâng cấp TOEIC 700 - 850 - #XX')
sql_lines.append("\n-- 2. Update Level 3 Upgrade Tests (level_id = 3, target 700-850)")
for i in range(1, 11):
    q_id = 180 + i
    q_code = f"UPGRADE-500-{i:02d}"
    q_title = escape_sql(f"Bài kiểm tra nâng cấp TOEIC 700 - 850 - #{i:02d}")
    q_desc = escape_sql(f"Bài kiểm tra nâng cấp trình độ TOEIC 700 - 850 - Đề số #{i:02d}. Đạt từ 80% (24/30 câu) để nâng cấp trình độ ngay lập tức.")
    sql_lines.append(
        f"UPDATE quizzes SET level_id = 3, quiz_code = '{q_code}', title = '{q_title}', description = '{q_desc}', quiz_category = 'LEVEL_UP', pass_score = 80, time_limit_seconds = 1800, total_questions = 30, is_active = 1 WHERE id = {q_id};"
    )

# 3. Update Existing Level 4 Upgrade Tests (IDs 191-200 -> level_id = 4, quiz_code = 'UPGRADE-650-01'..'10', Title = 'Bài kiểm tra nâng cấp TOEIC 850 - 990 - #XX')
sql_lines.append("\n-- 3. Update Level 4 Upgrade Tests (level_id = 4, target 850-990)")
for i in range(1, 11):
    q_id = 190 + i
    q_code = f"UPGRADE-650-{i:02d}"
    q_title = escape_sql(f"Bài kiểm tra nâng cấp TOEIC 850 - 990 - #{i:02d}")
    q_desc = escape_sql(f"Bài kiểm tra nâng cấp trình độ TOEIC 850 - 990 - Đề số #{i:02d}. Đạt từ 80% (24/30 câu) để nâng cấp trình độ ngay lập tức.")
    sql_lines.append(
        f"UPDATE quizzes SET level_id = 4, quiz_code = '{q_code}', title = '{q_title}', description = '{q_desc}', quiz_category = 'LEVEL_UP', pass_score = 80, time_limit_seconds = 1800, total_questions = 30, is_active = 1 WHERE id = {q_id};"
    )

# 4. Seed 10 New Level 1 Upgrade Tests (level_id = 1, quiz_code = 'UPGRADE-L1-01'..'10', Title = 'Bài kiểm tra nâng cấp TOEIC Dưới 500 - #XX')
sql_lines.append("\n-- 4. Seed 10 New Level 1 Upgrade Tests (level_id = 1, target Dưới 500)")

level1_cards = flashcards_by_level[1]

for i in range(1, 11):
    quiz_id_counter += 1
    current_quiz_id = quiz_id_counter
    q_code = f"UPGRADE-L1-{i:02d}"
    q_title = escape_sql(f"Bài kiểm tra nâng cấp TOEIC Dưới 500 - #{i:02d}")
    q_desc = escape_sql(f"Bài kiểm tra nâng cấp trình độ TOEIC Cơ bản (Dưới 500) - Đề số #{i:02d}. Đạt từ 80% (24/30 câu) để nâng cấp trình độ ngay lập tức.")
    ...
    sql_lines.append(
        ...
        f"VALUES ({current_quiz_id}, '{q_code}', '{q_title}', '{q_desc}', 'MULTIPLE_CHOICE', 1800, 80, 30, 1, 'LEVEL_UP', 1, 1, @admin_id, NOW(), NOW());"
    )
    
    # 30 questions from level 1 pool
    for q_idx in range(30):
        question_id_counter += 1
        current_question_id = question_id_counter
        
        card = level1_cards[(i * 3 + q_idx) % len(level1_cards)]
        w = escape_sql(card['word'])
        trans = escape_sql(card['translation'])
        ex = escape_sql(card['example'] or f"Study the word '{w}'.")
        ...
        distractors = [c for c in level1_cards if c['word'] != card['word']]
